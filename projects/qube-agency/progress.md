# Qube Agency - Current Progress

_Last updated: 2026-09-10_

## Strategic Frame (Badhan's model)
Two interdependent sides -- deal source and execution source. Progress tracked against both.

### Supply Chain Position (Sep 10 meeting clarity)
Qube is at **Level 4** in the AI data supply chain:
- Level 1: AI companies / labs (buy data directly)
- Level 2: Platforms (sell data to AI labs, run their own collection networks)
- Level 3: Tier 1/2 agencies (Cloudfactory, iMerit, Cogito, Taskmonk -- execute for platforms)
- Level 4: Qube (sub-executes for agencies)

**September agency KPI**: Break KGen dependency. Lock in 1 deal from a Level 3 agency independent of KGen.

### Agency Approach Correction (Zach, Sep 10)
Previous approach was **top-down** -- giving agencies a pilot deal to test if they can execute.
Correct approach is **bottom-up** -- intelligence first:
1. Find what deals are currently live with them
2. Identify who their vendors are and what they're executing
3. Understand what's coming next in their pipeline
4. Map all options they have for execution
5. Eliminate those options one by one until Qube is the only card left on the table
6. Only then pitch -- they buy because they have no other option at our price/speed

Long game: crack Level 3 agencies as sub-vendor → build relationships → get intro to Level 2 platforms → eventually reach AI labs directly.

---

## 5 Wings (BD + Execution)

### Wing 1: Direct Outreach (quality over volume, no ROI expected before November)
- AutoGTM DROPPED permanently (Sep 9 decision)
- Approach: low volume, high quality outreach to build relationships only
- Emails sent Sep 8: Diego Pousa (Train Them AI), Eddy Xu (Build AI), Shawn Shen (Memories.ai), Yugmai, Annolance AI
- **Shawn Shen (Memories.ai) replied Sep 9**: asked for email. Sent to eddy.wu@memories.ai + shawn.shen@memories.ai. Pitch: India egocentric data for visual memory / physical intelligence training.
- Viral Morzaria (viralstudios.io) follow-up sent Sep 9
- Drafts ready but unsent: Axis Robotics email, Sarvam AI DM
- New channel identified: LeRobot dataset uploaders on HuggingFace Hub (companies actively collecting robot training data)
  - yaak-ai (Yaak Technologies, Danish) -- spatial intelligence for forklifts/vehicles, India warehouses/ports angle
  - allenai (Allen Institute for AI) -- MolmoAct2 bimanual dataset
  - physical-intelligence -- already Twitter target
  - una-auxme -- active ROS2SmolVLA datasets (13 days ago)
  - SSU-RealityLab -- updated 2 days ago

### Wing 2: Agency Partnerships
- **NEW APPROACH (Sep 10)**: Bottom-up intel gathering. Don't lead with offering a deal. First: find what's live with them, who their vendors are, what's next, eliminate their alternatives.
- Atlas Capture via KGen: LIVE deal, 1-2k hours target Sep. Blocked on execution (no contributors sourced yet).
- Taskmonk: Reply sent, waiting for response
- iMerit: Received reply Thursday, sent response Monday. Price floor issue ($8 min CPA). **Intel angle**: their $8 floor = their vendor cost floor. Pitch we can execute the same deal below that.
- Cloudfactory: Call booking pending (Soham). **Next step**: intel call -- what's live, who they use, what's coming
- Cogito: Deal share pending (Soham). **Intel needed first** before sharing our deal
- Centific (Eduard K.): Warm intro via Abhishek Vadapalli -- not yet requested
- Other Tier 1/2 agency outreaches: Srijan + Soham sent Sep 8
- **To close something in September: bottom-up first, pilot pitch second**

### Wing 3: Individual Connection Building (LinkedIn + Twitter + warm intros)
- **LinkedIn Day 1 (Sep 9)**: 5 comments, 4 connections sent
  - Connected: Srikanth Vidapanakal (xSpecies AI), Sam Bigdeli (KavAI), Vadim Hortolomei (EMBODIOS), David Boardman (EveryPoint)
  - Comment only: Karol Hausman (Physical Intelligence, 3rd+)
  - Monthly notes: 0 remaining (used all 5 note slots)
  - Day 2 (Sep 10): Steven Fang (Orchestra Data) + Khurram P.
- **Twitter Day 1 (Sep 9)**: 13 engagements (6 replies + 7 follows). KPI (10+) hit.
  - Engaged: @axisrobotics, @adcock_brett, @chelseabfinn, @svlevine, @pabbeel, @deepakpathak, @RussTedrake
  - Day 2 targets queued (Sep 10): @DrJimFan, @drfeifei, @karpathy, @zipengfu, @SongShuran, @davheld, @HaochenShi74, @AndrewYNg, @axisrobotics, @tonyzzhang
- **Warm intros**: Eduard K. at Centific (Abhishek), Axis Robotics founder (Discord)
- **Sources for warm intros**: LinkedIn mutual connections, connection mapping conversations (Andros ex-Meta, Quinie at Flora Labs), Day 1 LinkedIn connections once accepted, Twitter reply-backs
- **LinkedIn post drafted**: Contributor recruitment post (looking for iPhone 11+ users in India for paid data collection)
- **LinkedIn bio update needed**: Change from Web3/crypto bio to "Operations Manager at Qube | Building India's data collection network for physical AI training | Working with teams in embodied AI, robotics, and multimodal AI"

### Wing 4: Community
- Strategy: Soham researches agency + AI company websites for Discord/Telegram/Reddit communities
- Join, check if active, stay active in active ones
- Add LeRobot Discord to the list (physical AI researchers + builders, exact ICP)
- Status: Not started yet

### Wing 5: Execution (September cash depends entirely on this)
- Atlas Capture requires iPhone 11+ contributors
- Target: 15-20 contributors x 3-4 approved hours/day x ~3 weeks = 1k+ hours
- **Blocker: Soham/Pranav networks have low iPhone penetration (~3-5% India average) + low CPA is not attractive**
- Alternative sourcing needed: LinkedIn post (drafted), urban professional networks, WhatsApp tech communities, Instagram iPhone users
- Nothing sourced yet as of Sep 9

---

## Deal Source

### What we have
- Atlas Capture deal (via KGen) -- live, 1-2k hours target this month
- Taskmonk: waiting for reply
- iMerit: price floor issue, negotiating
- Conversations open: Cloudfactory, Cogito (both waiting on Soham)
- Memories.ai: email sent Sep 9, warm lead
- Viral Morzaria (viralstudios.io): follow-up sent Sep 9, hot lead

### Issues
- All current deal flow traces to one source: KGen ecosystem. No independent channel proven yet.
- iMerit declined at $8 min CPA -- Tier 1 agencies have price floor above current margins.
- AutoGTM permanently dropped -- no automated outreach channel
- Wing 5 execution blocked: iPhone availability + low CPA for contributors

### Solutions in progress
- Pilot-first approach for agency closes: 200-500 hours, fast yes
- LeRobot dataset uploaders as quality outreach targets
- LinkedIn post for contributor recruitment
- Urban professional network sourcing for iPhone contributors

---

## This Week's Actions (Sep 8-15)
- [x] Follow up with Viral Morzaria -- sent Sep 9
- [x] Twitter Day 1 -- 13 engagements, KPI hit
- [x] LinkedIn Day 1 -- 5 comments, 4 connections
- [ ] LinkedIn Day 2 (Sep 10): Steven Fang + Khurram P.
- [ ] Twitter Day 2 (Sep 10): 10 targets queued in sheet
- [ ] Post LinkedIn contributor recruitment post
- [ ] Update LinkedIn bio
- [ ] Cloudfactory call -- Soham to book
- [ ] Share Atlas Capture deal with Cogito -- Soham
- [ ] Request warm intro to Eduard K. via Abhishek
- [ ] Source iPhone contributors via alternative channels (LinkedIn post, urban networks)
- [ ] Find yaak-ai + Allen AI contacts for quality LeRobot outreach

## PDCA Eval: Sep 11 (Thursday)
- Check Twitter reply-backs from Day 1 targets
- Check LinkedIn connection acceptances from Day 1
- Check if Taskmonk / Memories.ai respond
- Adjust angles if 0 engagements back on Twitter

---

## LinkedIn Outreach PDCA -- Sep 9 (Day 1)

Strategy: comment-then-connect. Max 5 comments/day, max 5 connections/day. No LinkedIn Premium (no DMs to non-connections).

### Completed today (Sep 9)
- Srikanth Vidapanakal (xSpecies AI): Commented on robotics post + Connected with note [0 notes remaining this month]
- Sam Bigdeli (KavAI): Commented on physical AI post + Connected without note
- Vadim Hortolomei (EMBODIOS): Commented on embodied AI post + Connected without note
- Karol Hausman (Physical Intelligence): Commented only (3rd+, Follow-only profile, no connect possible)
- David Boardman (EveryPoint): Commented on CDRA/stockpile post + Connected via ... menu without note (3rd+)

Daily limits hit: 5/5 comments, 4/5 connections

### Pending for Sep 10
- Steven Fang (Orchestra Data, row 15): Comment + connect
- Khurram P. (Unknown, row 16): Find recent post, comment + connect

---

## Twitter Outreach PDCA -- Sep 9 (Day 1)

Strategy: reply to + follow physical AI / robotics / egocentric data founders. Target 10+ engagements/day. Account: @Srijan0x (personal).

### Completed today (Sep 9)
- @axisrobotics (Axis Robotics): Replied to robotics thread + Following
- @adcock_brett (Brett Adcock, Figure AI CEO, 849.6K followers): Replied to robot autonomy tweet (66.1K views) + Following
- @chelseabfinn (Chelsea Finn, Physical Intelligence co-founder, 101.7K followers): Replied to pi07 tweet (73.1K views) + Following
- @svlevine (Sergey Levine, Physical Intelligence co-founder, 136.4K followers): Replied to behavioral cloning data tweet (49.2K views) + Following
- @pabbeel (Pieter Abbeel, Amazon FAR/Berkeley, 125.6K followers): Replied to OmniRetarget humanoid data tweet (104.6K views) + Following
- @deepakpathak (Deepak Pathak, SkildAI CEO/CMU, 30.1K followers): Replied to S1 in-context learning tweet (73.7K views) + Following
- @RussTedrake (Russ Tedrake, Walden Robotics/MIT, 6.5K followers): Following (no substantive tweet to reply to -- 36 posts total)

Total: 6 replies + 7 follows = 13 engagements. KPI (10+) hit.

Reply themes: data coverage/diversity as the real moat vs. scale alone; sim-to-real data gap; environmental diversity for generalization; quality + coverage needing to scale together.

### Day 2 Targets (Sep 10) -- queued in sheet rows 33-42
- @DrJimFan (Jim Fan, NVIDIA Director of Robotics, 585.6K)
- @drfeifei (Fei-Fei Li, World Labs CEO, 1M)
- @karpathy (Andrej Karpathy, 4.1M)
- @zipengfu (Zipeng Fu, Stanford AI Lab, 13.2K)
- @SongShuran (Shuran Song, Columbia Robotics)
- @davheld (David Held, Robotics Research)
- @HaochenShi74 (Haochen Shi, Robotics Research)
- @AndrewYNg (Andrew Ng, AI Fund)
- @axisrobotics (Axis Robotics -- reply to crowdsourced data post)
- @tonyzzhang (Tony Zhao, Stanford -- verify handle first)

---

## Cold Outreach Performance (Week of Sep 1-8)

| Campaign | Sent | Reply Rate | Note |
|---|---|---|---|
| AI product teams | 161 | 1.2% | Best -- 1 hot lead |
| AI data managers | 264 | 0.8% | Worst quality -- targeting fix needed |
| Growth marketers | 141 | 0.7% | Lower priority |
| AI startup founders | 166 | -- | Worth continuing |
| Consumer AI apps | 135 | -- | No replies yet |
| Regional agencies | 131 | -- | Cheapest, no data yet |
| **Total** | **998** | **0.5%** | 1 hot lead, $0 balance |

AutoGTM permanently dropped Sep 9. No further campaigns.

---

## GMV Performance (April-September 2026)

| Month | Plan | Actual | Cash In | Hit Rate |
|-------|------|--------|---------|----------|
| April | $20k | $9.1k | $7.3k | 45% |
| May | $21k | $7k | $0 | 33% |
| June | $18.4k | $12.25k | $500 | 67% |
| July | $20.1k | $4.5k | $0 | 22% |
| August | $12k | ~$5.3k | $0 | ~44% |
| September | TBD | In progress | $0 | -- |
